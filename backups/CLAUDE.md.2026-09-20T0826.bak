...
- **Well of Words** — Monitor + **Helper second-device view** (`?mode=helper`, shows answers, controls hints remotely via Firestore `wowSession/state` + `wowSession/cmd`, classroom-scoped by `?room=`). **Single cooperative class score** (no teams). Play: crossword grid with crossword-style numbers, stone-ring wheel (drag AND tap), free hint navigation (💬 meaning · 📖 verse **with QR** · 🖼 picture · **🔤+ repeatable one-more-letter** · 👁 reveal w/ confirm), Well Card with verse-anchored QR (`buildVerseUrl` derives `&id=pN#pN` from verseRef), 🪣 bonus bucket, **tried-words list** (client `TRIED_BLOCK` profanity scrub), 90s blitz, ⚙ scale gear incl. custom slider. Puzzles: **6–7 letters, 10–12 target words**, whitelist-only validation, client-side deterministic packer. v1 teen mode; 💧 Spring (K-6) plumbing hidden (v1.1).

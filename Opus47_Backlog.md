...
**Solution:** A second mode inside `games/memory.html` (one tier, ages ~4–11) — same 12 pairs, chunked client-side into 3 boards of 4 pairs; picture-first cards with the **emoji on both faces as the match key** so text is additive and never required; scripture in two registers (`verseSimple` plain-language line + verbatim `versePhrase` in quotes with its ref); a 4-block Story Card replacing the match modal; 🙌 Wiggle Break replacing Sabotage between boards. Content is **derived from the already-generated Youth pairs** by a cheap `/api/simplify-pairs` pass (Sonnet 4.6, small input) cached to `lessonLibrary/{id}.memoryPrimary` — no second lesson extraction, no new `gameType` in `pipeline.js`. Mode resolves from `?mode=` → classroom `audience` field → localStorage → launch picker. Full spec: `scripture-match-primary-design.md`.
**Acceptance:**
- `?mode=primary|youth` renders the right version; classroom `audience` field drives the default with no per-session picking
- Deterministic server checks: `versePhrase` is a normalised substring of the **source pair's** verse, and `verseRef`/`url`/`icon` are byte-identical to it (guards against the model rewriting scripture into kid language inside quotes, or drifting off the Youth card's reference)
- Icon collisions across the 12 inherited icons are detected and resolved — the emoji is the match key, so a duplicate makes a board unwinnable
